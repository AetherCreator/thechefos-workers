...
**Bible:** 1.1 (with §A7 + §A8 from the-pilgrimage applied)
**Substrate:** `hunter-exec.py` (post one-word patch 2026-05-06)
...
## Clue DAG — under §A7 + §A8

Every clue tagged with surface ([CHAT-OPUS] / [CODE-MANUAL] / [CODE-AUTONOMOUS]) AND reasoning class ([SYNTHESIS] / [DETERMINISTIC]) per Bible 1.2 §A8 candidate.

| # | Clue | Surface | Class | Description | Depends on |
|---|---|---|---|---|---|
| 1 | Pre-flight inventory | `[CODE-AUTONOMOUS]` | `[DETERMINISTIC]` | Verify infra readiness; produce `pre-flight-report.md`. **THE FIRE-AND-WALK-AWAY PROOF.** All shell steps STRICT-or-AUDIT-wrapped per §A7. ≤12 tool calls. <2 min wall-clock target. | none |
...
| 3 | Locke harvest worker | `[CODE-AUTONOMOUS]` | `[DETERMINISTIC]` | Build `packages/locke-harvest/` Cloudflare Worker. PROMPT carries the full worker source code verbatim — Hunter writes `src/index.ts`, `wrangler.toml`, package.json verbatim, commits, push triggers `deploy.yml` CI. Worker reads schema from C2, calls SearXNG + Gemini Flash, writes Lead JSON to brain/. Hourly cron. | C2 |
| 4 | Locke smoke test | `[CODE-AUTONOMOUS]` | `[DETERMINISTIC]` | Fire one harvest cycle (Telegram `/hunt` to @LockeLamoraBot OR direct curl to worker `/run`). Capture ≥1 valid Lead JSON in brain/05-leads/. PROMPT scopes single subreddit, ≤5 candidate threads, no Council step yet. | C3 |
...
| 6 | Council worker | `[CODE-AUTONOMOUS]` | `[DETERMINISTIC]` | Build `packages/council/` Worker. Hunter writes verbatim from PROMPT. Triggered by new lead in brain/05-leads/. Calls Ollama 3× (or until 1 of 3 abstains), computes geometric mean, writes verdict alongside lead. | C5 |
| 7 | Foundry pipeline | `[CODE-AUTONOMOUS]` | `[DETERMINISTIC]` | Build `packages/foundry/` Worker that orchestrates Schemer → Builder → Reviewer for Council-passed leads. Schemer (Gemini) writes a THDD MAP.md from a passed lead. Builder fires `/build` via existing Mastro path. Reviewer (Haiku) gates ship via Vercel deploy hook. PROMPT carries all 3 sub-handler bodies verbatim. | C6 |
| 8 | TREASURE + retro | `[CHAT-OPUS]` | `[SYNTHESIS]` | Validate fire-and-walk-away end-to-end. Empirical metrics: how many of C1/C3/C4/C6/C7 fired without Tyler bouncing? §A7 + §A8 violations? Bible 1.2 incorporation if everything held. | C7 |
...
- ✅ **Substrate match.** Bible 1.1 + §A7 + §A8 + hunter-exec.py post-patch — all consistent.
...
- **Per clue:** 1 attempt. If Hunter STUCK or PROMPT-author error surfaces, root-cause before retry. Use § A7+§A8 to diagnose: was the failure a missing wrap, a misclassified synthesis, or a substrate issue?
...
Both share: hunter-exec.py + Mastro dispatch (orchestration), Gemini Flash API (free research), Agent-Reach (structured web scraping), SearXNG (self-hosted search), Ollama (free local inference), Cloudflare Workers (APIs), GitHub (all outputs).
...
| Code execution | **hunter-exec.py via /build** | $0 marginal (NIM Nemotron via free-cc-proxy) |
...
SCHEMER (Gemini Flash)
  Writes THDD MAP.md + clue-1 PROMPT for the new product
       │
       ▼
BUILDER (hunter-exec.py via /build)
