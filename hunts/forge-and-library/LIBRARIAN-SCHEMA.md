...
**Status:** v1.0 (locked at C2, synthesis from `LOCKE-LAMORA-SOUL.md` + `prompts/HUNTER.md`)
...
1. **`{PERSONA}-SOUL.md`** — voice, principles, taste. Example: `LOCKE-LAMORA-SOUL.md`. Loaded into Gemini system prompt at hunt time.
2. **`prompts/{PERSONA}.md`** — concrete query plan and analysis instructions. Example: `prompts/HUNTER.md`. Loaded into Gemini user-prompt template at hunt time.
...
**(a) Cron, declared in `wrangler.toml`.** Persona-defined. Locke runs `0 0 * * 0` (Sunday midnight UTC) per HUNTER.md §Hunt Schedule. A future Layer-1 knowledge harvester might run `0 5 * * *` (5 AM UTC daily). The Worker reads its own cron context to log the trigger source.
...
Source mediation is centralized. Hunter Workers do NOT hit external APIs directly. They go through three centralized adapters running on InfiniVeg:

- **SearXNG** at `http://searxng-tunnel.thechefos.app/search` (Cloudflare-tunneled, 4-hour cache) — Google, Brave, DDG, Startpage meta-search.
- **Agent-Reach** at `http://agent-reach.thechefos.app/extract` — Reddit, YouTube, RSS, web extraction. Install pending; C1 audit confirms.
- **Gemini Flash** at `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent` — synthesis, classification, persona narration.

Each adapter has a fixed request/response shape. Hunter Workers compose calls but never mint new adapter logic. New source types (e.g., Twitter/X) require an adapter PR, not a Worker change.
...
- **Hunt session reports:** `brain/05-leads/_sessions/{persona}-{ISO_TIMESTAMP}.json` — one per `/run`, includes all candidate URLs scanned, kept-vs-discarded counts, total Gemini tokens used.
...
3. **Gemini quota** — 50 requests per `/run` hard ceiling (Gemini Flash free tier is 1,000/day across the project; reserve headroom for Council).
...
- **Generic-pattern reject** — Gemini outputs containing "everyone", "all developers", "many users" without specificity get auto-rejected by validator (see LOCKE-OUTPUT-SCHEMA §2).
...
- `harvest_complete` — total_leads, gemini_tokens, wall_clock_seconds, exit_status

Tyler reads aggregate via `GET /api/intel/summary?days=7&persona=locke`.

## 10. Failure Modes

- **Gemini 429** → exponential backoff (1s, 4s, 16s) + abort at 3rd retry. Session marked `quota_partial`.
...
For Locke specifically, the contract is `LOCKE-OUTPUT-SCHEMA.md` (sibling document). Worker imports a generated `zod` schema from that doc and pipes every Gemini response through it before any brain/ write.

## 12. Cost Ceiling

Per-run budgets (enforced):

- Gemini: 50 requests × ~500 input tokens × ~250 output tokens — within free tier.
- Workers CPU: 8-min wall-clock × ~50ms cron CPU avg → negligible.
- KV reads/writes: <100/run.
- D1 inserts: <50/run.

Monthly target: ~$0.50 across all hunter personas (rounded $9 hunt budget covers it 18× over). Cost ceiling enforcement: any single `/run` exceeding 100 Gemini calls triggers a `cost_breach` alert via intel_log + immediate exit.
...
This schema is **immutable v1.0** for the duration of this hunt's clue-3 through clue-7 builds. Revisions land as v1.1+ at C8 retro if empirical data demands it.
