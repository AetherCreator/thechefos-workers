...
Goal: A Model Context Protocol server at thechefos.app/api/mcp that Claude Chat can connect to as a custom connector. At session start, Claude loads full context automatically — no token required.
...
- Tyler adds thechefos.app/api/mcp as a custom connector in Claude settings
...
4. **Claude Connector Test** → pass: Tyler adds thechefos.app/api/mcp in Claude settings → Tools, session start auto-loads ACTIVE-STATE and OPS-BOARD content
