...
app.post('/github/:operation', async (c) => {
  ...
  try {
    ...
    const branch = (params.branch as string | undefined) || (params.ref as string | undefined) || 'main'
    if (op === 'get_file') {
      const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${path}?ref=${branch}`, { headers: { ...ghHeaders, Accept: 'application/vnd.github.v3.raw' } })
      if (!res.ok) throw new Error(`GitHub ${res.status}: ${await res.text()}`)
      data = await res.text()
    } else if (op === 'put_file') {
      const content = params.content as string | undefined
      const body: Record<string, unknown> = { message: (params.message as string | undefined) || 'Update via proxy', content: Buffer.from(content ?? '', 'utf8').toString('base64') }
      if (params.sha) body.sha = params.sha
      if (params.branch) body.branch = params.branch
      const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${path}`, { method: 'PUT', headers: ghHeaders, body: JSON.stringify(body) })
      if (!res.ok) throw new Error(`GitHub ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else if (op === 'list_dir') {
      ...
      const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/branches?per_page=${perPage}`, { headers: ghHeaders })
      if (!res.ok) throw new Error(`GitHub ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else if (op === 'create_pr') {
      const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/pulls`, { method: 'POST', headers: ghHeaders, body: JSON.stringify({ title: (params.message as string | undefined) || 'PR via proxy', head: params.head, base: (params.base as string | undefined) || 'main', body: '' }) })
      if (!res.ok) throw new Error(`GitHub ${res.status}: ${await res.text()}`)
      data = await res.json()
      ...
      const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/actions/runs?per_page=${perPage}`, { headers: ghHeaders })
      if (!res.ok) throw new Error(`GitHub ${res.status}: ${await res.text()}`)
      data = await res.json()
...
app.post('/cloudflare/:operation', async (c) => {
  ...
  const cfHeaders: Record<string, string> = { Authorization: `Bearer ${c.env.CF_API_TOKEN}`, 'Content-Type': 'application/json' }
  ...
  try {
    ...
    if (op === 'workers_list') {
      ...
      const res = await fetch(`${base}/d1/database/${params.database_id}/query`, { method: 'POST', headers: cfHeaders, body: JSON.stringify({ sql: params.sql, params: (params.params as unknown[]) || [] }) })
      if (!res.ok) throw new Error(`CF ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else if (op === 'kv_get') {
      const res = await fetch(`${base}/storage/kv/namespaces/${params.namespace_id}/values/${params.key}`, { headers: cfHeaders })
      if (!res.ok) throw new Error(`CF ${res.status}: ${await res.text()}`)
      data = await res.text()
    } else if (op === 'kv_set') {
      const res = await fetch(`${base}/storage/kv/namespaces/${params.namespace_id}/values/${params.key}`, { method: 'PUT', headers: { ...cfHeaders, 'Content-Type': 'text/plain' }, body: params.value as string })
      if (!res.ok) throw new Error(`CF ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else if (op === 'kv_list') {
      const res = await fetch(`${base}/storage/kv/namespaces/${params.namespace_id}/keys`, { headers: cfHeaders })
      if (!res.ok) throw new Error(`CF ${res.status}: ${await res.text()}`)
      data = await res.json()
...
app.post('/vercel/:operation', async (c) => {
  ...
  const vHeaders: Record<string, string> = { Authorization: `Bearer ${c.env.VERCEL_TOKEN}`, 'Content-Type': 'application/json' }
  try {
    let data: unknown
    if (op === 'list_projects') {
      const res = await fetch(`https://api.vercel.com/v9/projects?teamId=${teamId}&limit=20`, { headers: vHeaders })
      ...
      const res = await fetch(`https://api.vercel.com/v13/deployments/${params.deployment_id}?teamId=${teamId}`, { headers: vHeaders })
      if (!res.ok) throw new Error(`Vercel ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else if (op === 'get_runtime_logs') {
      const since = (params.since as number | undefined) || Date.now() - 3_600_000
      const res = await fetch(`https://api.vercel.com/v2/deployments/${params.deployment_id}/events?teamId=${teamId}&since=${since}&limit=100`, { headers: vHeaders })
      if (!res.ok) throw new Error(`Vercel ${res.status}: ${await res.text()}`)
      data = await res.text()
...
app.post('/calendar/:operation', async (c) => {
  ...
  try {
    ...
    const gHeaders: Record<string, string> = { Authorization: `Bearer ${access_token}`, 'Content-Type': 'application/json' }
    const calId = encodeURIComponent((params.calendar_id as string | undefined) || 'primary')
    let data: unknown
    if (op === 'list_calendars') {
      const res = await fetch('https://www.googleapis.com/calendar/v3/users/me/calendarList', { headers: gHeaders })
      if (!res.ok) throw new Error(`Calendar ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else if (op === 'list_events') {
      const qs = new URLSearchParams({ timeMin: (params.time_min as string | undefined) || new Date().toISOString(), timeMax: (params.time_max as string | undefined) || new Date(Date.now() + 7 * 86_400_000).toISOString(), singleEvents: 'true', orderBy: 'startTime', maxResults: '20' })
      if (params.query) qs.set('q', params.query as string)
      const res = await fetch(`https://www.googleapis.com/calendar/v3/calendars/${calId}/events?${qs}`, { headers: gHeaders })
      if (!res.ok) throw new Error(`Calendar ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else if (op === 'create_event') {
      const res = await fetch(`https://www.googleapis.com/calendar/v3/calendars/${calId}/events`, { method: 'POST', headers: gHeaders, body: JSON.stringify(params.event) })
      if (!res.ok) throw new Error(`Calendar ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else if (op === 'find_free_time') {
      const res = await fetch('https://www.googleapis.com/calendar/v3/freeBusy', { method: 'POST', headers: gHeaders, body: JSON.stringify({ timeMin: params.time_min, timeMax: params.time_max, items: [{ id: (params.calendar_id as string | undefined) || 'primary' }] }) })
      if (!res.ok) throw new Error(`Calendar ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else {
...
app.post('/gmail/:operation', async (c) => {
  ...
  try {
    ...
    if (op === 'search_messages') {
      const qs = new URLSearchParams({ q: (params.q as string | undefined) || '', maxResults: String((params.max_results as number | undefined) || 10) })
      const res = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages?${qs}`, { headers: gHeaders })
      if (!res.ok) throw new Error(`Gmail ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else if (op === 'get_message') {
      const res = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${params.message_id}?format=full`, { headers: gHeaders })
      if (!res.ok) throw new Error(`Gmail ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else if (op === 'get_profile') {
      const res = await fetch('https://gmail.googleapis.com/gmail/v1/users/me/profile', { headers: gHeaders })
      if (!res.ok) throw new Error(`Gmail ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else if (op === 'get_thread') {
      const res = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/threads/${params.thread_id}?format=full`, { headers: gHeaders })
      if (!res.ok) throw new Error(`Gmail ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else {
...
app.post('/valtown/:operation', async (c) => {
  const op = c.req.param('operation')
  const params = await c.req.json<Record<string, unknown>>()
  const vtHeaders: Record<string, string> = { Authorization: `Bearer ${c.env.VALTOWN_TOKEN}`, 'Content-Type': 'application/json' }
  ...
  try {
    ...
    if (op === 'me') {
      ...
      const fileRes = await fetch(`${base}/v2/vals/${val.id}/files?path=index.ts`, {
        method: 'POST', headers: vtHeaders,
        body: JSON.stringify({ content: params.code, type: fileType }),
      })
      if (!fileRes.ok) {
      ...
      const res = await fetch(`${base}/v2/vals/${params.val_id}/files?path=index.ts`, {
        method: 'PUT', headers: vtHeaders,
        body: JSON.stringify({ content: params.code, type: params.type }),
      })
      if (!res.ok) throw new Error(`ValTown ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else if (op === 'delete_val') {
      const res = await fetch(`${base}/v2/vals/${params.val_id}`, { method: 'DELETE', headers: vtHeaders })
      if (!res.ok) throw new Error(`ValTown ${res.status}: ${await res.text()}`)
      data = { deleted: true }
    } else if (op === 'run_val') {
      ...
      const res = await fetch(`${base}/v1/sqlite/execute`, {
        method: 'POST', headers: vtHeaders,
        body: JSON.stringify({ statement: { sql: params.sql, args: params.args || [] } }),
      })
      if (!res.ok) throw new Error(`ValTown ${res.status}: ${await res.text()}`)
      data = await res.json()
    } else {
